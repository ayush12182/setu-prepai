...
const content = fs.readFileSync('/Users/ayushdixit12/Downloads/PrepEntrance_Mathematics_Complete_2100_Questions.md', 'utf-8');
...
for (const chapterText of chapters) {
  ...
  if (chapterName.includes('PrepEntrance') || chapterName.includes('Mathematics --- Complete') || chapterName.includes('Master Answer Key')) continue;
  ...
  for (const qBlock of qBlocks) {
    ...
    if (answer) {
      questions.push({
        ...
        exam_type: 'JEE_MAINS',
        question_type: 'MCQ',
        source: 'PrepEntrance 2100 Maths'
      });
    }
  }
}
...
console.log(`Successfully added ${questions.length} Maths questions. New total: ${newTotal.length} questions in realQuestionBank.json`);
