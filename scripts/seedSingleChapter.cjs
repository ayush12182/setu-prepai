...
exports.seedSingleChapter = seedSingleChapter;
...
async function seedSingleChapter(subject, chapterName, topics, supabase, geminiKey) {
...
NO MARKDOWN, NO EXPLANATION outside the JSON.`;
    try {
        ...
        for (const f of data.formulas) {
            if (!f.title || !f.latex || !f.concept || !f.variables)
                continue; // Required fields
            if (f.latex.includes('$'))
                continue; // Invalid latex syntax containing unescaped wrappers
            const titleKey = f.title.toLowerCase().trim();
            const latexKey = f.latex.replace(/\s+/g, '');
            if (seenTitles.has(titleKey) || seenLatex.has(latexKey))
        ...
        const { data: chapterRes, error: chapErr } = await supabase
            ...
            .select()
            .single();
        if (chapErr || !chapterRes)
            throw new Error(`Metadata Upsert Failed: ${chapErr?.message}`);
        ...
        if (insertErr)
            throw new Error(`Formulas Insert Failed: ${insertErr.message}`);
