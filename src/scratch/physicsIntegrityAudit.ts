...
import { physicsChapters } from '../data/syllabus';

export function runPhysicsIntegrityAudit() {
  console.log("=== STARTING PHYSICS CURRICULUM GRAPH INTEGRITY AUDIT ===");
  ...
  const concepts = loadJson(conceptsPhysPath);
  const formulas = loadJson(formulasPhysPath);
  ...
  concepts.forEach((c: any) => {
    ...
    if (!c.subject || c.subject !== 'physics') {
      traceabilityFailures.push(`Concept ${c.concept_id}: missing/incorrect subject '${c.subject}'`);
    }
    
    const matchedChapter = physicsChapters.find(ch => ch.name.toLowerCase() === c.chapter?.toLowerCase());
    if (!matchedChapter) {
  ...
  // Calculate Physics Integrity Score
  let score = 100;
  ...
  const mdPath = path.join('/Users/ayushdixit12/.gemini/antigravity-ide/brain/1405db37-c8c4-4a16-89f1-2056d6cdcae0/', 'physics_integrity_audit.md');

  const reportContent = `# Physics Curriculum Graph Integrity Audit Report
...
- **Total Physics Concepts**: ${concepts.length}
- **Total Physics Formulas**: ${formulas.length}
- **Total Misconceptions Checked**: ${Object.keys(misconceptions).length}
...
`;
  ...
  console.log(`[Audit] Physics Integrity report successfully written to ${mdPath}`);
  ...
  if (score < 95 || duplicateConceptIds.length > 0 || duplicateConceptNames.length > 0 || orphanConcepts.length > 0 || missingFormulaLinks.length > 0 || invalidMisconceptionCounts.length > 0 || missingMisconceptionTypes.length > 0 || missingPyqFields.length > 0) {
    throw new Error(`Physics Integrity Audit Failed. Score: ${score}. Review report at ${mdPath}`);
  }
}

// Invoke automatically if run directly
if (process.argv[1] && process.argv[1].includes('physicsIntegrityAudit')) {
