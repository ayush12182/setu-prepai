...
import { chemistryChapters } from '../data/syllabus';

export function runChemistryIntegrityAudit() {
  console.log("=== STARTING CHEMISTRY CURRICULUM GRAPH INTEGRITY AUDIT ===");
  ...
  const concepts = loadJson(conceptsChemPath);
  const formulas = loadJson(formulasChemPath);
  ...
  concepts.forEach((c: any) => {
    ...
    if (!c.subject || c.subject !== 'chemistry') {
      traceabilityFailures.push(`Concept ${c.concept_id}: missing/incorrect subject '${c.subject}'`);
    }
    
    const matchedChapter = chemistryChapters.find(ch => ch.name.toLowerCase() === c.chapter?.toLowerCase());
    if (!matchedChapter) {
  ...
  // Calculate Chemistry Integrity Score
  let score = 100;
  ...
  const mdPath = path.join('/Users/ayushdixit12/.gemini/antigravity-ide/brain/1405db37-c8c4-4a16-89f1-2056d6cdcae0/', 'chemistry_integrity_audit.md');

  const reportContent = `# Chemistry Curriculum Graph Integrity Audit Report
...
- **Total Chemistry Concepts**: ${concepts.length}
- **Total Chemistry Formulas**: ${formulas.length}
...
`;
  ...
  console.log(`[Audit] Chemistry Integrity report successfully written to ${mdPath}`);
  ...
  if (score < 95 || duplicateConceptIds.length > 0 || duplicateConceptNames.length > 0 || orphanConcepts.length > 0 || missingFormulaLinks.length > 0 || invalidMisconceptionCounts.length > 0 || missingMisconceptionTypes.length > 0 || missingPyqFields.length > 0) {
    throw new Error(`Chemistry Integrity Audit Failed. Score: ${score}. Review report at ${mdPath}`);
  }
}

// Invoke automatically if run directly
if (process.argv[1] && process.argv[1].includes('chemistryIntegrityAudit')) {
  runChemistryIntegrityAudit();
}
