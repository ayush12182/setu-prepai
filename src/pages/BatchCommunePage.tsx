...
function BatchCommuneArena({ roomId, batchInfo, roster }: { roomId: string, batchInfo: any, roster: any[] }) {
  ...
  const handleSend = async () => {
    ...
    if (currentText.toLowerCase().startsWith('@prepentrance')) {
      ...
      const aiReply = await askSetu(question);
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${
                  ...
                  isSetu ? 'bg-blue-600/20 text-blue-100 border border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.15)]' :
